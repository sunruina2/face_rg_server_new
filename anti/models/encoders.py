...
from anti.models.backbones.mobilenet import MobileNet
from anti.models.backbones.resnet import ResNet
from anti.models.backbones.resnet import BasicBlock
from anti.models.backbones.resnet import Bottleneck
from anti.models.backbones.senet import SENet
from anti.models.backbones.senet import SEBottleneck
from anti.models.backbones.senet import SEResNetBottleneck
from anti.models.backbones.senet import SEResNeXtBottleneck
from anti.models.backbones.densenet import DenseNet

from anti.models.blocks import build_layers
...
def se_resnet50(device='cpu', out_features=1, pretrained=False, *argv, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(
            model_zoo.load_url('http://data.lip6.fr/cadene/pretrainedmodels/se_resnet50-ce0d4300.pth'))
...
def se_resnext50(device='cpu', out_features=1, pretrained=False, *argv, **kwargs):
    model = SENet(SEResNeXtBottleneck, [3, 4, 6, 3], groups=32, reduction=16,
                  ...
                  **kwargs)
    if pretrained:
        model.load_state_dict(torch.load('models/se_resnext50_32x4d-a260b3a4.pth'))
        # model.load_state_dict(model_zoo.load_url('http://data.lip6.fr/cadene/pretrainedmodels/se_resnext50_32x4d-a260b3a4.pth'))

    model.logsigma = nn.Parameter(torch.FloatTensor([-0.5, -0.5]))
    # Feature map:
    model.cnn4 = nn.Conv2d(in_channels=256, out_channels=128, kernel_size=3, stride=1, padding=1)
    model.cnn5 = nn.Conv2d(in_channels=128, out_channels=3, kernel_size=3, stride=1, padding=1)
    model.cnn6 = nn.Conv2d(in_channels=3, out_channels=1, kernel_size=3, stride=1, padding=1)
    # Depth map:
    model.cnn7 = nn.Conv2d(in_channels=256, out_channels=128, kernel_size=3, stride=1, padding=1)
    model.cnn8 = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=3, stride=1, padding=1)
    model.cnn9 = nn.Conv2d(in_channels=64, out_channels=1, kernel_size=3, stride=1, padding=1)
    model.resize_32 = nn.Upsample(size=32, mode='nearest')
    model.last_linear = nn.Linear(model.last_linear.in_features, out_features)
    return model.to(device)
...
def se_resnext101(device='cpu', out_features=1, pretrained=False, *argv, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(
            model_zoo.load_url('http://data.lip6.fr/cadene/pretrainedmodels/se_resnext101_32x4d-3b2fe3d8.pth'))
