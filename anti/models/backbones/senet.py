...
class SENet(nn.Module):
    ...
    def __init__(self, block, layers, groups, reduction, dropout_p=0.2,
        ...
        #Feature map:
        #self.cnn4=nn.Conv2d(in_channels=256,out_channels=128, kernel_size=3,stride=1,padding=1)
        ...
        #Depth map:
        #self.cnn7=nn.Conv2d(in_channels=256,out_channels=128, kernel_size=3,stride=1,padding=1)
    ...
    def features(self, x):
        ...
        #print(2222,x.size())
        x = self.layer1(x)
        #print(33333,x.size())#8, 256, 64, 64
    ...
    def forward(self, x):
        T,D = self.features(x)
        b=x.size()[0]
        #V=torch.where(D>=0.1,torch.ones(b,1,32,32).cuda(),torch.zeros(b,1,32,32).cuda()) 
        V=torch.where(D>=0.1,torch.ones(b,1,32,32).to(x.device),torch.zeros(b,1,32,32).to(x.device))
        #V = D
        U=T*V
        #print('U',U.size())
        x = self.layer3(U)
        ...
        #print('layer4',x.size())
        x = self.logits(x)
        #print('final',x.size(),D.size())
        return x,D,self.logsigma
